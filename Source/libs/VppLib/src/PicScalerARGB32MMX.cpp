...
MODULE				: PicScalerARGB32MMX

FILE NAME			: PicScalerARGB32MMX.cpp
...
NOTE:	This code replaces PicScalerARGB32MMX.cpp and must not be compilled together with this file.
...
extern "C" 
{
  void ScaleRowAsmARGB32MMX(void *pDst, unsigned _widthOut, void *const*pSrcRows, unsigned _widthIn);
  void EmitEMMS(void);
  void EmitFSAVE(unsigned char *Statex87);
  void EmitFRSTOR(unsigned char *Statex87);
}
...
*/
int PicScalerARGB32MMX::Scale(void* pOutImg, const void* pInImg, bool VFlip)
{
  if(pOutImg==NULL || pInImg==NULL || _widthIn==0 || _heightIn==0)
   ...
   if(VFlip)
   {
     ...
     while(y-- > 0)
     {
       ...
       ScaleRowAsmARGB32MMX(pDst, _widthOut, (void*const*)pSrcRows, _widthIn);
